...
def run_cdp(project_directory, json_directory, log_directory, video_routes, scraping_function, pull_from_database, database_head, versioning_head, relevant_tfidf_storage_key, commit_to_database, delete_videos=False, delete_splits=False, test_search_term='bicycle infrastructure', prints=True, block_sleep_duration=21600, run_duration=-1, logging=True):
    ...
    try:
        ...
        while (time_elapsed + block_sleep_duration) <= run_duration or run_duration == -1:
            ...
            block['completed_feeds'] = len(get_video_feeds(packed_routes=video_routes, storage_directory=json_directory, scraping_function=scraping_function, prints=prints))
            ...
            if (type(prior_stored_data) is collections.OrderedDict) or (type(prior_stored_data) is dict):
                generate_tfidf_from_directory(transcript_directory=(project_directory + 'transcripts/'), storage_directory=json_directory, stored_data=prior_stored_data[relevant_tfidf_storage_key], prints=prints)
            else:
                generate_tfidf_from_directory(transcript_directory=(project_directory + 'transcripts/'), storage_directory=json_directory, stored_data=None, prints=prints)
