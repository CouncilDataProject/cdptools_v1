    ...
    try:
        ...
        while (time_elapsed + block_sleep_duration) <= run_duration or run_duration == -1:
            ...
            if (type(prior_stored_data) is collections.OrderedDict) or (type(prior_stored_data) is dict):
                generate_tfidf_from_directory(project_directory=(project_directory + 'transcripts/'), storage_directory=json_directory, stored_data=prior_stored_data[relevant_tfidf_storage_key], prints=prints)
            else:
                generate_tfidf_from_directory(project_directory=(project_directory + 'transcripts/'), storage_directory=json_directory, stored_data=None, prints=prints)
